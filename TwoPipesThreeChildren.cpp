...
// Purpose: CS3376
// Description:
//	This program executes "ls -ltr | grep 3376 | wc -l" using two pipes and three child processes.
...
int main(int argc, char **argv) {
	...
	if(childpid1 == 0) {	//First child process.
		...
		if(childpid2 == 0) {	//Second child process.
			...
			//Fork to create third child (to execute wc).
			if((childpid3 = fork()) == -1) {
				perror("Error creating a child process.");
				exit(1);
			}
			
			if(childpid3 == 0) {	//Third child process.
